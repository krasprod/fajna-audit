...
- **Token:** `8264822499:AAHcUzxu8A2gT4ML_sWKoeGBWC4x9KoDdIQ` (у `data.json`, публічно)
- **Group:** «Файна Перукарня звіт», chat_id: `-5141489549`
...
2. 📩 Нова відповідь від клієнта — коли клієнт натиснув «Надіслати відповідь» на запиті
3. 💰 Етап оплачено — коли адмін позначив етап оплаченим
4. ✅ Етап завершено — коли всі задачі етапу стали `done`

Клієнт-форма шле відповідь через **Formsubmit.co** (email на krasprod.nk@gmail.com) + одночасно сповіщення у Telegram-групу.

⚠ Токен у публічному JSON → теоретично може використовуватись для спаму в групу. Якщо таке станеться — `/revoke` у @BotFather і замінити токен у `data.json` + `chat_id` (через `getUpdates`).
